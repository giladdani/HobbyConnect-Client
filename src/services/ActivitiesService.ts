...
async function fetch_activities() {
    const response = await fetch(`${process.env.REACT_APP_SERVER_BASE_URL}/api/activities`,
    {
        headers: {
...
async function fetch_activity_by_id(id:string) {
    const response = await fetch(`${process.env.REACT_APP_SERVER_BASE_URL}/api/activities/${id}`,
    {
        headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/json'
...
async function create_activity(activity:Activity, userToken:string) {
    const response = await fetch(`${process.env.REACT_APP_SERVER_BASE_URL}/api/events`,
    {
        headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/json',
